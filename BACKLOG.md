...
| A6 | Add Dodo webhook `https://<domain>/api/webhooks/payments` (test endpoint) → copy `DODO_PAYMENTS_WEBHOOK_KEY` | owner | owner | **Staging verified (partial)**: signed success, failed payment, tax-inclusive amount handling, stale/wrong-amount refunds, and endpoint delivery reached the hosted endpoint; full matrix remains. Subscribe to `payment.succeeded/failed/cancelled` |
