...
4. Use free logs and free uptime checks initially. Vercel Hobby currently has no available Log Drain or alert-rule destination (`Add Drain`, `Add Rule`, and `Add Webhook` are disabled), so external monitoring wiring is **External provider blocked**; the hosted health endpoints remain available for an external uptime check.
