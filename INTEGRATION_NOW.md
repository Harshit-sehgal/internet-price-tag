...
6. Run real signed sandbox transactions and the full payment-state matrix. **Staging verified (partial)** for successful and declined payments, signed webhook delivery, duplicate-event replay/idempotency, provider replay of a successful event with a new HTTP 200 delivery, synthetic provider `payment.cancelled` delivery with HTTP 200, cancelled-checkout UI behavior, quote consumption, atomic takeover finalization, tax-inclusive provider payload handling, and stale/wrong-amount refunds; a real customer-cancellation state transition plus race/refund-failure, missing-metadata, and outage cases remain outstanding.
