...
| Item | Status | Evidence |
|---|---|---|
| Supabase project + migrations + auth + Realtime + backups | Staging verified (auth/health/Realtime); Owner/provider blocked for backup verification | Existing Priced project and hosted-hardening migrations are the source of truth; Site URL and `/auth/callback` are configured, Google login reaches the welcome flow, `@harshit` is saved, and `/api/health?check=db` is healthy. A live two-session Realtime market update is verified. Supabase explicitly shows that Free Plan does not include project backups, and the pre-money logical dump test remains blocked by authenticated database access. See `INTEGRATION_NOW.md`. |
...
5. **Backup test**: create and test the documented logical backup procedure before accepting real customer money; Supabase Free Plan has no project backups and the logical dump is currently Owner blocked by authenticated database access. Do not enable PITR during the free beta phase.
