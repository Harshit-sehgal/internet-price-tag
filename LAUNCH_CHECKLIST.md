...
| Real-Postgres RPC concurrency (10 + 25 racers) | CI verified; Owner blocked for hosted verification | Dockerized Postgres tests pass locally/CI; the hosted `tests/integration/postgres.finalize.test.ts` run requires the protected service-role key and the 10/25 hosted challenger race remains outstanding |
...
1. **Supabase/Auth** (§1): the hosted logical schema/data dump and isolated PostgreSQL 17 restore are complete; Google OAuth, callback, welcome, logout/re-login, `@harshit` handle creation, and the live two-session Realtime update are complete. The hosted service-role RPC test remains outstanding.
